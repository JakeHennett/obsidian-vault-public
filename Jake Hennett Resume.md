**Address:** Enoree, SC 29335  
**Phone:** 864-345-9764
**Email:** [Jake.Hennett@gmail.com](mailto:Jake.Hennett@gmail.com)
**LinkedIn:** [LinkedIn Profile](https://www.linkedin.com/in/jake-hennett-69024764/)
**GitHub:** [GitHub Profile](https://github.com/JakeHennett)

---

## Summary

Software Engineer and manager with more than 12 years’ experience with proprietary products utilized by the healthcare, manufacturing, and financial industries. Extensive background in product design implementation and software product support.

---

## Job Experience

### Software Engineering Manager — *Lima One Capital*  
**Greenville, SC** — *September 7, 2021 – January 15, 2026*

- Leads a team of software engineers and QA technicians  
- Cross-team collaboration with internal and offshore teams across multiple projects
- Writes and maintains MuleSoft and NestJS middle-tier APIs for internal/external use  
- Developed a Pricing Engine SPA for generating loan quotes  
- Integrated third-party services via RESTful APIs  
- Deploys and services API packages, troubleshooting as needed  
- Delivers inventive solutions to satisfy business needs and preserve system integrity  
- Manages and updates PostgreSQL databases using pgAdmin  
- Participates in Scrum ceremonies to ensure timely, quality delivery  

---

### Deployment Specialist — *Michelin (Contract DevOps Engineer with THG)*  
**Greenville, SC** — *May 5, 2021 – September 3, 2021*

- Deployed and configured Cameleon software at Michelin facilities  
- Maintained Oracle databases, updating records to reflect equipment and PLC system changes  

---

### Software Engineer — *System Logistics Corporation*  
**Arden, NC** — *August 25, 2020 – April 27, 2021*

- Developed and tested multiple software projects  
- Provided second-level support for escalated customer issues  

---

### Software Development Engineer II — *RedSail Technologies*  
**Spartanburg, SC** — *July 23, 2012 – July 31, 2020*

- Managed third-party REST API package for customer pharmacies  
- Created and updated APIs, interfacing with flat files via assembly applications  

---

## Education

### Southern New Hampshire University  
**Master of Science** — *September 2017 - December 2023*  
Major: Information Technology  
Focus: Software Application Development  

### University of South Carolina Upstate  
**Bachelor of Science** — *August 2009 - May 2013*  
Major: Computer Science  
Minor: Mathematics  

---

## Presentations

**Carolina Code Conference** — *August 2024*  
[The Value of Prototypes (YouTube)](https://youtu.be/HjBgcPWRYT8?si=6gaofjxa5hx3qpxz)
[Conference Profile](https://blog.carolina.codes/i/144339321/jake-hennett-greenville-sc)
